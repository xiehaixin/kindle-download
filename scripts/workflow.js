    ...
    try {
        ...
        if (!hasAuthor && !hasPublisher && filtered.length > 1) {
            ...
            if (uniqueAuthors.length > 1 || uniquePublishers.length > 1) {
                ...
                // 保存状态
                const state = { bookName, baseUrl, results, browserStateFile: browserStateFile, timestamp: Date.now() };
                fs.writeFileSync(stateFile, JSON.stringify(state, null, 2));
                console.log("LOG: 已保存浏览器登录态和搜索状态");
                console.log("SAVE_DIR:" + saveDir);
                console.log("SEARCH_RESULTS_JSON:" + JSON.stringify(filtered.slice(0, 10).map(r => ({ title: r.titleText, author: r.author, publisher: r.publisher, rating: r.rating }))));
